var game = new Phaser.Game(1280, 1024, Phaser.CANVAS, 'phaser-example', { create: create, update: update, render: render });
...
var arrayMultiplier = 1.5;

multiplyArray = function(array, multiplier) {
    var i;
    var newArray = [];
...
outlineVertices = multiplyArray(outlineVertices, arrayMultiplier);
launcherVertices = multiplyArray(launcherVertices, arrayMultiplier);
guide1Vertices = multiplyArray(guide1Vertices, arrayMultiplier);
guide2Vertices = multiplyArray(guide2Vertices, arrayMultiplier);
guide3Vertices = multiplyArray(guide3Vertices, arrayMultiplier);
guide4Vertices = multiplyArray(guide4Vertices, arrayMultiplier);
gutterVertices = multiplyArray(gutterVertices, arrayMultiplier);
bouncer1 = multiplyArray(bouncer1, arrayMultiplier);
bouncer2 = multiplyArray(bouncer2, arrayMultiplier);
smallCircles = multiplyArray(smallCircles, arrayMultiplier);
mediumCircles = multiplyArray(mediumCircles, arrayMultiplier);
largeCircles = multiplyArray(largeCircles, arrayMultiplier);
leftFlipperVertices = multiplyArray(leftFlipperVertices, arrayMultiplier);
rightFlipperVertices = multiplyArray(rightFlipperVertices, arrayMultiplier);
ballStart = multiplyArray(ballStart, arrayMultiplier);

var ballBody;
var flipperJoints = [];
var PTM = 100 * arrayMultiplier; // conversion ratio for values in arrays above
var needReset = false;

function create() {
    
    game.world.setBounds(-400 * arrayMultiplier, -520 * arrayMultiplier, 800 * arrayMultiplier, 600 * arrayMultiplier);
    
    game.stage.backgroundColor = '#0070AD';
    ...
    // ball
    game.physics.box2d.restitution = 0.1;
    ballBody = new Phaser.Physics.Box2D.Body(this.game, null, ballStart[0] * (PTM / arrayMultiplier), ballStart[1] * (PTM / arrayMultiplier));
    ...
    cursors = {
        "sideButtonLeft": game.input.keyboard.addKey(Phaser.Keyboard.C), // 1SW7
        "sideButoonRight": game.input.keyboard.addKey(Phaser.Keyboard.J), // 2SW7
    };
    
    var caption = game.add.text(5, 5, 'Score: ', { fill: '#ffffff', font: '20pt Arial' });
...
function update() {
    ...
    if (cursors.sideButtonLeft.isDown)
    {
    ...
    if (cursors.sideButoonRight.isDown)
    {
